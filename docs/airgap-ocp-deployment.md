...
This guide walks through deploying the Instana Agent Operator on an OpenShift cluster
...
If you do not use icr.io, skip this step and make sure that you have a target registry prepared.
...
Create a Kubernetes pull secret with the same IAM API key used during mirroring and link
...
- relies on the namespace-wide pull secret set up in [Deploy the Operator on OpenShift](#deploy-the-operator-on-openshift)
