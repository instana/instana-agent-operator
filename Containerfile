...
RUN export BUILDER_ARCHITECTURE="$(echo ${TARGETPLATFORM} | cut -d'/' -f2)" && ./installGolang.sh ${GO_VERSION} ${BUILDER_ARCHITECTURE}
...
RUN make install-tools generate-deepcopies generate-manifests
