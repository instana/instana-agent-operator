...
func (e *envBuilder) build(envVar EnvVar) *corev1.EnvVar {
	...
	case MavenRepoFeaturesPath:
		return fromCRField("INSTANA_MVN_REPOSITORY_FEATURES_PATH", e.agent.Spec.Agent.MvnRepoFeaturesPath)
	case MavenRepoSharedPath:
		return fromCRField("INSTANA_MVN_REPOSITORY_SHARED_PATH", e.agent.Spec.Agent.MvnRepoSharedPath)
	case ProxyHostEnv:
...
// fromCRField assumes a custom resource field to be passed and creates an *EnvVar if conditions are met
func fromCRField[T any](name string, val T) *corev1.EnvVar {
	valueString := fmt.Sprintf("%v", val)
	if valueString == "" { // Omitted fields will appear as empty strings
		return nil
	}
	return &corev1.EnvVar{Name: name, Value: valueString}
}
