...
func TestAggregatedClusterRoleBindingBuilder_IsNamespaced_ComponentName(t *testing.T) {
	assertions := require.New(t)

	crb := NewAggregatedClusterRoleBindingBuilder(nil)

	assertions.False(crb.IsNamespaced())
	assertions.Equal(constants.ComponentK8Sensor, crb.ComponentName())
}

func TestAggregatedClusterRoleBindingBuilder_Build(t *testing.T) {
	assertions := require.New(t)
	...
	expected := optional.Of[client.Object](
		&rbacv1.ClusterRoleBinding{
			...
			RoleRef: rbacv1.RoleRef{
				...
				Kind:     "ClusterRole",
				Name:     agentName + "-aggregated-k8sensor",
			},
	...
	crb := &aggregatedclusterRoleBindingBuilder{
		InstanaAgent: agent,
		Helpers:      helpers,
	}
