...
func (r *RemoteAgentReconciler) cleanupHelmChart(
	ctx context.Context,
	...
	if !controllerutil.RemoveFinalizer(agentNew, finalizerV1) {
		...
		log.Error(err, "failed to uninstall deprecated Helm installation of remote agent")
		return reconcileFailure(err)
	} else {
		log.V(1).Info("successfully uninstalled deprecated Helm installation of remote agent")
