...
LATEST_RELEASE_BRANCH=$(git branch -r | grep 'origin/release-' | sed 's/origin\///' | sort -V | tail -1)
...
echo "Checking out latest release branch to temporary branch $TEMP_BRANCH..."
...
if [ ! -f "ci/FEDRAMP_VERSION" ]; then
  echo "Error: ci/FEDRAMP_VERSION file not found in $LATEST_RELEASE_BRANCH"
...
# Output the version for use in scripts
